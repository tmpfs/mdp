...
Readme.prototype.option = function() {
  this
    ...
    .option('--text',
      'Use a text renderer, default output is README.txt')
    .option('--man',
      'Use a man renderer, default output is README.1')
    .option('-t, --title [title]', 'Document title')
    .option('-d, --metadata [file...]', 'Meta definition file(s)',
...
Readme.prototype.on = function() {
  this
    .on('complete', function(req) {
      ...
      var files = this.metadata, defaults = files.length == 0;
      if(!files.length) {
      ...
      read.call(cli, files, function(err, meta) {
        ...
        if(!output) {
          // TODO: use title as filename?
          ofile = meta.output
            ? meta.output : (cli.text ? 'README.txt' : 'README.md');
          if(!meta.output && cli.man) {
            ofile = 'README.1';
          }
        ...
        concat.call(cli, meta, function(err, source) {
          ...
          tokenize.call(cli, meta, source, function(err, tokens) {
            if(err) return done(err);
            //console.dir(tokens);
            build.call(cli, formats.map.txt, meta, tokens, function(err, document) {
              if(err) return done(err);
